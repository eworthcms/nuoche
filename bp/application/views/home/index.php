...
<div class="gtCon">
  <div class="tips">
    我的账户情况，今日费用：<strong class="red"><?php echo $spend_money; ?></strong> 元 账户余额：<strong class="red"><?php echo $remain_sum; ?></strong> 元.
  </div> 
  <div class="srcArea">
...
var myChart = new Chart(ctx, {
    ...
    data: {
        ...
        datasets: [{
            ...
            borderWidth: 1,
            data : [<?php foreach ($report_field as $k=>$val) { $cos = $val['cost']?sprintf("%1\$.2f", $val['cost']/100).',':'0.00,'; $num = substr($cos,0,-1); echo $num.',';}?>]
        }]
    },
    options: {
		title: {
    ...
    <div id='fenyeCon'>
      <table width="100%" border="0" cellpadding="0" cellspacing="0" class="table">
        <thead>
          <tr>
            <th>推广计划名称</th>
            <?php if(isset($costs) && $costs){?><th style="position: relative; ">花费&nbsp;<?php if($order_field=='cost' && $order_type=='desc') {?><i class="desc_sort<?php if(isset($cost_sort) && $cost_sort=='desc'){?>_new<?php }?>" onclick="report_sort('campaign', 'cost', 'asc')"></i><?php }else{?><i class="asc_sort<?php if(isset($cost_sort) && $cost_sort=='asc'){?>_new<?php }?>" onclick="report_sort('campaign', 'cost', 'desc')"></i><?php }?></th><?php }?>
            <?php if(isset($click_cost) && $click_cost){?><th style="position: relative; ">平均点击花费&nbsp;<?php if($order_field=='average_cost' && $order_type=='desc') {?><i class="desc_sort<?php if(isset($average_cost_sort) && $average_cost_sort=='desc'){?>_new<?php }?>" onclick="report_sort('campaign', 'average_cost', 'asc')"></i><?php }else{?><i class="asc_sort<?php if(isset($average_cost_sort) && $average_cost_sort=='asc'){?>_new<?php }?>" onclick="report_sort('campaign', 'average_cost', 'desc')"></i><?php }?></th><?php }?>
        ...
        <?php foreach ($campaign as $k=>$val) {?>
          <tr>
            <td><span onclick="find_adgroup(<?php echo $val['id']; ?>)"><a><?php echo $val['campaign_name']?></a></span></td>
            <?php if(isset($costs) && $costs){?>
    ...
    <div id='fenyeCon'>
      <table width="100%" border="0" cellpadding="0" cellspacing="0" class="table">
        <thead>
          <tr>
            <th>推广组名称</th>
            <?php if(isset($costs) && $costs){?><th style="position: relative; ">花费&nbsp;<?php if($order_field=='cost' && $order_type=='desc') {?><i class="desc_sort<?php if(isset($cost_sort) && $cost_sort=='desc'){?>_new<?php }?>" onclick="report_sort('adgroup', 'cost', 'asc')"></i><?php }else{?><i class="asc_sort<?php if(isset($cost_sort) && $cost_sort=='asc'){?>_new<?php }?>" onclick="report_sort('adgroup', 'cost', 'desc')"></i><?php }?></th><?php }?>
            <?php if(isset($click_cost) && $click_cost){?><th style="position: relative; ">平均点击花费&nbsp;<?php if($order_field=='average_cost' && $order_type=='desc') {?><i class="desc_sort<?php if(isset($average_cost_sort) && $average_cost_sort=='desc'){?>_new<?php }?>" onclick="report_sort('adgroup', 'average_cost', 'asc')"></i><?php }else{?><i class="asc_sort<?php if(isset($average_cost_sort) && $average_cost_sort=='asc'){?>_new<?php }?>" onclick="report_sort('adgroup', 'average_cost', 'desc')"></i><?php }?></th><?php }?>
        ...
        <?php foreach ($adgroup as $k=>$val) {?>
          <tr>
            <td><span onclick="find_adinfo(<?php echo $val['id']; ?>)"><a><?php echo $val['adgroup_name']?></a></span></td>
            <?php if(isset($costs) && $costs){?>
            <td><span class="org"><?php echo $val['cost']; ?></span>元</td>
            <?php }?>
            <?php if(isset($click_cost) && $click_cost){?>
    ...
    <div id='fenyeCon'>
      <table width="100%" border="0" cellpadding="0" cellspacing="0" class="table">
        <thead>
          <tr>
            <th>广告标题</th>
            <?php if(isset($costs) && $costs){?><th style="position: relative; ">花费&nbsp;<?php if($order_field=='cost' && $order_type=='desc') {?><i class="desc_sort<?php if(isset($cost_sort) && $cost_sort=='desc'){?>_new<?php }?>" onclick="report_sort('adinfo', 'cost', 'asc')"></i><?php }else{?><i class="asc_sort<?php if(isset($cost_sort) && $cost_sort=='asc'){?>_new<?php }?>" onclick="report_sort('adinfo', 'cost', 'desc')"></i><?php }?></th><?php }?>
            <?php if(isset($click_cost) && $click_cost){?><th style="position: relative; ">平均点击花费&nbsp;<?php if($order_field=='average_cost' && $order_type=='desc') {?><i class="desc_sort<?php if(isset($average_cost_sort) && $average_cost_sort=='desc'){?>_new<?php }?>" onclick="report_sort('adinfo', 'average_cost', 'asc')"></i><?php }else{?><i class="asc_sort<?php if(isset($average_cost_sort) && $average_cost_sort=='asc'){?>_new<?php }?>" onclick="report_sort('adinfo', 'average_cost', 'desc')"></i><?php }?></th><?php }?>
        ...
        <?php foreach ($adinfo as $k=>$val) {?>
          <tr>
            <td><?php echo $val['title']?><a href=""></a></td>
            <?php if(isset($costs) && $costs){?>
            <td><span class="org"><?php echo $val['cost']; ?></span>元</td>
            <?php }?>
            <?php if(isset($click_cost) && $click_cost){?>
...
$(document).ready(function() {
	...
	function report_sort(report_type, order_field, sort) {
		...
		location.href = '/home/index/'+report_fid+'/'+report_type+'/'+time+'/'+start_time+'/'+end_time+'/'+order_field+'/'+sort+'/1';
	}

	//查找指定推广计划下的推广组
	function find_adgroup(report_fid) {
		location.href='/adgroup/adgroup_list/'+report_fid+'/0/id/desc/<?php echo isset($cur_page)?$cur_page:1; ?>'
	}
