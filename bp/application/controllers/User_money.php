...
class User_money extends MY_Controller {
    ...
    public function index($start_time=0, $end_time=0, $cur_page=1)
    {
    	$this->money_list($start_time, $end_time, $cur_page);
    }
    public function money_list($start_time=0, $end_time=0, $cur_page=1)
	{
	    ...
	    $config['total_rows'] = $this->user_money_service->get_money_log_count($conditions, $this->session->userdata('id'));
	    ...
	    $data['money'] = $this->user_money_service->get_user_money($conditions, $this->session->userdata('id'));
	...
	public function update_password()
	{
	    ...
	    if($this->input->post())
	    {
	        ...
	        $this->user_money_service->update_password($password);
	...
	{
	    $this->load->service('user_money_service');
	    $res = $this->user_money_service->check_password($old_password, $this->session->userdata('id'));
	...
	public function download()
	{
	    $this->load->service('user_money_service');
	    $conditions = array();
	    $money_log = $this->user_money_service->get_user_money($conditions, $this->session->userdata('id'));
