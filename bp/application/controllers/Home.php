...
class Home extends MY_Controller {
	...
	public function index($report_fid=0, $report_type='campaign', $time=0, $start_time=0, $end_time=0, $order_field='id', $order_type='desc', $cur_page=1)
	{
	    ...
	    $this->load->service('campaign_service');
	    $this->load->service('adgroup_service');
	    $this->load->service('adinfo_service');
	    $this->load->service('ad_report_service');
	    $this->load->service('user_money_service');
	    ...
	    if($data['report_type']=='campaign')
	    {
	        ...
	        $data['campaign'] = $this->campaign_service->get_campaign($conditions, $this->session->userdata('id'));
	        // dd($data['campaign']);
    	    //获取推广计划资源数据
    	    if($data['campaign'])
    	    {
        	    ...
        	    foreach ($data['campaign'] as $k=>$val)
        	    {
        	        ...
        	        {
        	            $report = $this->ad_report_service->get_report_time($val['id'], 0, 0, $data['start_time'], $data['end_time']);
        	        ...
        	        {
        	            $report = $this->ad_report_service->get_report($val['id'], 0, 0, $report_time);
        	    ...
        	    // dd($data['campaign']);
        	    // usort 排序
        	    usort($data['campaign'],function($a,$b)
        	    {
        	        $value_a = $value_b = 0;
        	    ...
        	    if($report_time < strtotime('-30 day') && $report_time > strtotime('-31 day'))
        	    {
        	        for($s=1;$s<=30;$s++)
        	        {
        	           ...
        	           $data['report_field'][] = $this->ad_report_service->get_report_type($campaign_id, 0, 0, $data['report_type'], $t_time + $s_hours, $t_time + $e_hours);
        	        }
        	    }
        	    ...
        	    if(isset($data['start_time']) && $data['start_time'] && isset($data['end_time']) && $data['end_time'])
        	    {
        	        ...
        	        if($custom_time <= 30 && $custom_time > 7)
        	        {
        	            for($i=0;$i<=$custom_time - 1;$i++)
        	            {
        	                ...
        	                $data['report_field'][] = $this->ad_report_service->get_report_type($campaign_id, 0, 0, $data['report_type'], $s_time, $e_time);
        	            }
        	        }
	    ...
	    if($data['report_type']=='adgroup')
	    {
	        ...
	        $data['adgroup'] = $this->adgroup_service->get_adgroup($conditions, $this->session->userdata('id'));
	        ...
	        if($data['adgroup'])
	        {
    	        ...
    	        foreach ($data['adgroup'] as $k=>$val)
    	        {
    	            if(isset($data['start_time']) && $data['start_time'] && isset($data['end_time']) && $data['end_time'])
    	            {
    	                $report = $this->ad_report_service->get_report_time(0, $val['id'], 0, $data['start_time'], $data['end_time']);
    	            ...
    	            {
    	                $report = $this->ad_report_service->get_report(0, $val['id'], 0, $report_time);
    	                $data['adgroup'][$k]['click'] = $report['clicks']?$report['clicks']:0;
    	                $data['adgroup'][$k]['impressions'] = $report['impressions']?$report['impressions']:0;
    	                $data['adgroup'][$k]['cost'] = sprintf("%1\$.2f", $report['cost']/100);
    	                $ctr = 0;
    	        ...
    	        // usort 排序
    	        usort($data['adgroup'],function($a,$b)
    	        {
    	            $value_a = $value_b = 0;
    	            $value_a = $a[$this->order_field];
    	            $value_b = $b[$this->order_field];
    	        ...
    	        //分页
    	        $data['adgroup'] = array_slice($data['adgroup'],$offset,$page_size);
    	        //推广组投放效果趋势图数据
    	        $data['report_field'] = array();
    	        if($report_time < strtotime('-30 day') && $report_time > strtotime('-31 day'))
    	        {
    	            for($s=1;$s<=30;$s++)
    	            {
    	                ...
    	                $e_hours = $s * 86400;
    	                $t_time = strtotime(date('Y-m-d 00:00:00', strtotime('-30 day')));
    	                $data['report_field'][] = $this->ad_report_service->get_report_type(0, $adgroup_id, 0, $data['report_type'], $t_time + $s_hours, $t_time + $e_hours);
    	            }
    	        }
    	        if($report_time < strtotime('-7 day') && $report_time > strtotime('-30 day'))
    	        {
    	        ...
    	        if(isset($data['start_time']) && $data['start_time'] && isset($data['end_time']) && $data['end_time'])
    	        {
    	            ...
    	            if($custom_time <= 30 && $custom_time > 7)
    	            {
    	                for($i=0;$i<=$custom_time - 1;$i++)
    	                {
    	                    ...
    	                    $s_time = strtotime($data['start_time']) + $s_day;  //开始的时间戳
    	                    $e_time = strtotime($data['start_time']) + $e_day;  //后一天时间戳
    	                    $data['report_field'][] = $this->ad_report_service->get_report_type(0, $adgroup_id, 0, $data['report_type'], $s_time, $e_time);
    	                }
    	            }
    	            if($custom_time <= 7 && $custom_time > 1)
    	            {
	    ...
	    if($data['report_type']=='adinfo')
	    {
	        ...
	        //获取广告资源数据
	        $data['adinfo'] = $this->adinfo_service->get_adinfo($conditions, $this->session->userdata('id'));
	        ...
	        if($data['adinfo'])
	        {
    	        ...
    	        foreach ($data['adinfo'] as $k=>$val)
    	        {
    	            if(isset($data['start_time']) && $data['start_time'] && isset($data['end_time']) && $data['end_time'])
    	            {
    	                $report = $this->ad_report_service->get_report_time(0, 0, $val['id'], $data['start_time'], $data['end_time']);
    	            ...
    	            {
    	                $report = $this->ad_report_service->get_report(0, 0, $val['id'], $report_time);
    	                $data['adinfo'][$k]['click'] = $report['clicks']?$report['clicks']:0;
    	                $data['adinfo'][$k]['impressions'] = $report['impressions']?$report['impressions']:0;
    	                $data['adinfo'][$k]['cost'] = sprintf("%1\$.2f", $report['cost']/100);
    	                $ctr = 0;
    	        ...
    	        // usort 排序
    	        usort($data['adinfo'],function($a,$b)
    	        {
    	            $value_a = $value_b = 0;
    	            $value_a = $a[$this->order_field];
    	            $value_b = $b[$this->order_field];
    	        ...
    	        //分页
    	        $data['adinfo'] = array_slice($data['adinfo'],$offset,$page_size);
    	        //广告投放效果趋势图数据
    	        $data['report_field'] = array();
    	        if($report_time < strtotime('-30 day') && $report_time > strtotime('-31 day'))
    	        {
    	            for($s=1;$s<=30;$s++)
    	            {
    	                ...
    	                $e_hours = $s * 86400;
    	                $t_time = strtotime(date('Y-m-d 00:00:00', strtotime('-30 day')));
    	                $data['report_field'][] = $this->ad_report_service->get_report_type(0, 0, $adinfo_id, $data['report_type'], $t_time + $s_hours, $t_time + $e_hours);
    	            }
    	        }
    	        if($report_time < strtotime('-7 day') && $report_time > strtotime('-30 day'))
    	        {
    	        ...
    	        if(isset($data['start_time']) && $data['start_time'] && isset($data['end_time']) && $data['end_time'])
    	        {
    	            ...
    	            if($custom_time <= 30 && $custom_time > 7)
    	            {
    	                for($i=0;$i<=$custom_time - 1;$i++)
    	                {
    	                    ...
    	                    $s_time = strtotime($data['start_time']) + $s_day;  //开始的时间戳
    	                    $e_time = strtotime($data['start_time']) + $e_day;  //后一天时间戳
    	                    $data['report_field'][] = $this->ad_report_service->get_report_type(0, 0, $adinfo_id, $data['report_type'], $s_time, $e_time);
    	                }
    	            }
    	            if($custom_time <= 7 && $custom_time > 1)
    	            {
	    ...
	    $data['money'] = $this->user_money_service->get_money($today, $this->session->userdata('id'));
	    $data['spend'] = '';
	    $data['user_money'] = $this->user_money_service->get_my_money();
