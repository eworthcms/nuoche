...
class User_log extends MY_Controller {
    ...
    public function index($user=0, $content=0, $start_time=0, $end_time=0, $cur_page=1)
    {
    	$this->log_list($user, $content, $start_time, $end_time, $cur_page);
    }
    ...
    public function log_list($user=0, $content=0, $start_time=0, $end_time=0, $cur_page=1)
	{
	    ...
	    $this->load->service('user_log_service');
	    $conditions = array();
	    ...
	    $config['total_rows'] = $this->user_log_service->get_user_log_count($conditions);
	    ...
	    $data['user_log'] = $this->user_log_service->get_user_log($conditions);
